...
/**
 * A simple sorted stream example. Windows are used to collect events.
 * If clearOnTrigger value is set (i.e. if it's a tumbling window) then the window is only sorted on output.
 * Otherwise, the window is kept in sorted order always.
 */
  ...
  public List<Flow> getFlows() {
    Flow flow = new FlowBuilder()
      ...
      .stream("stream1")
        ...
        .sort().sortBy("count").tumbling(Policy.COUNT, 20).end()
      .endStream()
      .endDefs()
    .createFlow();
