...
/**
 * An example showing how the StopGate(快速闸门) flow op works.
 * Conceptually this can be thought of as a governor(节速器,管理者), where,
 * untill a particular condition is met, events flow freely through it.
 * However, when an activation condition(激活条件) is triggered, the gate closes and all events are dropped.
 * When an close condition(封闭状态) is met, the gate is lifted(解除) and events can pass through once again
 */
