 ...
 * <p/>
 * Run drpc query as
 * java -cp .:`storm classpath`:$REPO/mlstorm/target/mlstorm-00.01-jar-with-dependencies.jar BptiEnsembleQuery $DRPC_SRVER EnsembleClusterer /$BPTI_REPO/bpti_db/features
 */
    ...
    public static void main(final String[] args) throws IOException, TException, DRPCExecutionException {
        ...
        while (scanner.hasNextLine() && i++ < 1) {
            ...
            for (Map<String, List<Double>> map : dict) {
                ...
                final String parameters = MlStormFeatureVectorUtils.serializeFeatureVector(ArrayUtils.toPrimitive(both));
