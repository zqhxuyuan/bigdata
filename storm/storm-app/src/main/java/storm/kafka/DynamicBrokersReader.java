...
/**
 * 动态读取Brokers信息. Brokers信息写在ZooKeeper上.
 * 因为Kafka集群的节点是允许当掉的, 所以ZooKeeper上记录的Brokers信息是动态的.
 */
public class DynamicBrokersReader {
    ...
    private String _topic;  // 只针对一个Topic
    ...
    //分区列表是针对某个主题的, 即一个主题在集群中的所有分区信息都写在ZK中
    public String partitionPath() {
        return _zkPath + "/topics/" + _topic + "/partitions";
    }

    //broker路径是集群中的Kafka节点, 定义在server.properties中的broker.id配置项
     ...
     *
     * 某个主题中的每个分区都有个角色为leader的服务器，可以有0到多个角色为follower的服务器
     * 该方法分解分区编号, 获取这个分区的leader
     * @param partition 分区编号
     * @return 这个分区的leader节点
     */
