...
public class RankableObjectWithFields implements Rankable, Serializable {
  ...
  public static RankableObjectWithFields from(Tuple tuple) {
    //获取tuple内的数据有多种方式:
    //tuple.getValue(0);  (Long)tuple.getValue(1)
    //tuple.getValueByField("obj"); (Long)tuple.getValueByField("count")
    //tuple实际上是有三个字段的, 即RollingCountBolt的"obj", "count", "actualWindowLengthInSeconds"
    //这里去掉了前两个字段, 最后otherFields剩下actualWindowLengthInSeconds
  ...
  //topN排序的依据count的值.所以比较的字段为count.
  @Override
