 ...
 *    So it stores in the database how many words have appeared 0-9 times,
 *    how many have appeared 10-19 times, and so on.
 *    单词的出现次数为[0-9]的有几个, 出现次数为[10-19]的有几个单词
 *
 *    单词的出现次数,保存在第一个Bolt里.
 *    在第二个Bolt里,根据出现的次数,判断要放在哪个Bucket里.
 *    如果放在其中的一个Bucket里,则那一个Bucket只加了1,并从前一个的Bucket数量减1
 * <p/>
 * A batch of words can cause the bucket counts to decrement for some buckets
 * and increment for others as words move between buckets as their counts accumulate(逐渐增加).
 */
public class TransactionalWords {
    ...
    public static class KeyedCountUpdater extends BaseTransactionalBolt implements ICommitter {
        Map<String, Integer> _counts = new HashMap<String, Integer>(); // word-count map
        ...
        public void finishBatch() {
            for (String key : _counts.keySet()) {
                ...
                // key is the word, count和prev_count分别代表本次批处理前后的数量
    ...
    public static class Bucketize extends BaseBasicBolt {
        ...
        public void execute(Tuple tuple, BasicOutputCollector collector) {
            ...
            //假设BUCKET_SIZE=10,如果出现次数少于10,会被分配到第一个BUCKET中,因为cur<10,cur/10=0
            ...
            //如果之前在preBucket中,现在在currBucket中,且这两个bucket不相等,则要移动
            else if (currBucket != prevBucket) {
                collector.emit(new Values(attempt, currBucket, 1)); //当前+1
                collector.emit(new Values(attempt, prevBucket, -1));//之前-1
            }
        }
