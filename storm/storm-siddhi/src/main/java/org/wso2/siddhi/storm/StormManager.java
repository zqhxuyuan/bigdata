...
import org.wso2.siddhi.storm.components.SiddhiProcessingBolt;
import org.wso2.siddhi.storm.components.SiddhiSpout;
...
public class StormManager {
    ...
    private List<SiddhiProcessingBolt> SiddhiProcessingBoltList = new ArrayList<SiddhiProcessingBolt>();
...
/*
    ...
    public Map<String, SiddhiProcessingBolt> instantiateStormComponents(ExecutionPlanConfiguration executionPlanConfiguration, ExecutionPlanReference executionPlanReference) throws StormConfigurationException {
        executionPlanReferenceList.add(executionPlanReference);
        Map<String, SiddhiProcessingBolt> SiddhiProcessingBoltsForQuery = new ConcurrentHashMap<String, SiddhiProcessingBolt>();
        ...
        for (String queryReference : executionPlanReference.getQueryReferenceList()) {
            ...
            SiddhiProcessingBolt SiddhiProcessingBolt = new SiddhiProcessingBolt(queryReference, inputStreamId, new String[]{outputStreamId},
                    getTopLevelStreamId(inputStreamId, executionPlanConfiguration));
            SiddhiProcessingBoltsForQuery.put(topLevelOutputStreamId, SiddhiProcessingBolt);
        }
        SiddhiProcessingBoltList.addAll(SiddhiProcessingBoltsForQuery.values());
        return SiddhiProcessingBoltsForQuery;
    }
*/
...
/*
        ...
        for (SiddhiProcessingBolt SiddhiProcessingBolt : SiddhiProcessingBoltList) {
            BoltDeclarer boltDeclarer = builder.setBolt(getBoltId(SiddhiProcessingBolt.getInputSiddhiStreamId()), SiddhiProcessingBolt, 3);
            EventJunction eventJunction = StormProcessorValueHolder.getStormProcessorService().getEventJunction(SiddhiProcessingBolt.getImportedTopLevelStreamId(), tenantId);
            ...
            for (EventProducer eventProducer : eventJunction.getAllEventProducers()) {
                if (eventProducer instanceof SiddhiProcessingBolt) {
                    String inStreamId = ((SiddhiProcessingBolt) eventProducer).getInputSiddhiStreamId();
                    for(String outStreamId: ((SiddhiProcessingBolt)eventProducer).getOutputSiddhiStreamIds()) {
