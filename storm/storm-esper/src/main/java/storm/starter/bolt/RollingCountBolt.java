...
public class RollingCountBolt extends BaseRichBolt {
    ...
    public RollingCountBolt(int windowLengthInSeconds) {
        this(windowLengthInSeconds, DEFAULT_EMIT_FREQUENCY_IN_SECONDS);
    }
    ...
    public RollingCountBolt(int windowLengthInSeconds, int emitFrequencyInSeconds, int fieldSize) {
    ...
    private void countObjAndAck(Tuple tuple) {
        ...
        if(fieldSize==1) {
            obj = tuple.getValue(0);
        }else if(fieldSize==2){
            Object left = tuple.getValue(0);
            Object right = tuple.getValue(1);
            obj = new Pair(left, right);
        }else {
            obj = tuple.getValues();
        }
