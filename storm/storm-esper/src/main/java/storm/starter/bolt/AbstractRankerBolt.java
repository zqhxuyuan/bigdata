...
public abstract class AbstractRankerBolt extends BaseBasicBolt {
  ...
  //注意这个时间和客户端设置的滑动窗口时间大小没有关系,这是用于中间Bolt的缓存.
  //可以根据业务需求自定义一个默认值, 当然也可以在topology中传递第二个参数给IntermediateRankingsBolt.
   ...
   * 和RollingCountBolt.execute的处理方式一样,
   * 如果不是tick tuple,则更新窗口中某一项tuple的数据,
   * 如果是tick tuple,则发送窗口内所有项的统计数据
   */
  @Override
