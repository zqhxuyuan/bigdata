...
/** Analyses a streaming dataset of web page views. This class demonstrates several types of
  ...
  * `$ bin/run-example org.apache.spark.examples.streaming.clickstream.PageViewStream popularUsersSeen localhost 44444`
  */
// scalastyle:on
object PageViewStream {
  def main(args: Array[String]) {
    ...
    val pageViews = ssc.socketTextStream(host, port)
                       .flatMap(_.split("\n"))
                       .map(PageView.fromString(_))  // 一条条的点击纪录

    //----------metrics----------

    // Return a count of views per URL seen in each batch 每个页面的点击次数-->统计网站的PV
    ...
    val statusesPerZipCode = pageViews.window(Seconds(30), Seconds(2))
                                      ...
                                      .groupByKey()  // 根据区域分组, 同一个区域的所有记录都会归类到一组中
    //根据区域分组后,统计访问网站的出错率
    val errorRatePerZipCode = statusesPerZipCode.map{
      case(zip, statuses) =>
        ...
        if (errorRatio > 0.05) {
          "%s: **%s**".format(zip, errorRatio)  // 5%的出错率,Ops,赶紧告警
        } else {
    ...
    val activeUserCount = pageViews.window(Seconds(15), Seconds(2))
                                   ...
                                   .groupByKey()  //同一个User会有多条记录
                                   .count() //对于同一个User的多条记录,只统计一次!
    ...
    metric match {
      ...
      case "popularUsersSeen" =>
        ...
        pageViews.map(view => (view.userID, 1))  // original rdd=pageViews, change it to PairRDDFunctions
            //rdd should be PairRDDFunctions, where key is userId->THAT's previous mapper doing!
            //the joined rdd:userList also has key:userId. so we can join together
            .foreachRDD((rdd, time) => rdd.join(userList)
            //(K,V) join (K,W) become (K,(V,W)). we only focus W
            .map(_._2._2)
            .take(10)
            .foreach(u => println("Saw user %s at time %s".format(u, time))))
            //Saw user Reynold Xin at time 1436321190000 ms
            //Saw user Patrick Wendell at time 1436321230000 ms
